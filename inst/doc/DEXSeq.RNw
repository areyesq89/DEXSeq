...
\author{Alejandro Reyes\\ Simon Anders\\ Wolfganf Huber}
...
\begin{abstract}
  ...
  between different conditions.  For the analysis differential
  ...
  as input the number of reads falling on each of the exons of a
...
Brooks et al.~\cite{Brooks2010} investigated the effect of siRNA knock-down of Pasilla, a gene 
...
genes of Drosophila melanogaster.  A subset was chosen in order to
speed up the computations shown in this vignette. We start by loading
the \Rpackage{DEXSeq} package and the example data.
...
%--------------------------------------------------
\section{Normalisation and dispersion estimation}
%--------------------------------------------------
...
as in \Rpackage{DESeq} using the function \Rfunction{estimateSizeFactors}. The size factors
...
One might vary the formula to contemplate other sources of noise (see Section \ref{4}).
%
<<estDisp1,cache=TRUE>>=
