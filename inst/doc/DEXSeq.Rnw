...
In this section, we describe how to create an \Rclass{ExonCountSet} from an alignment of the RNA-seq reads 
in SAM format and file describing gene and transcript models in GTF format.

The first two steps of this workflow involve two scripts for the Python library \Rpackage{HTSeq}.
...
and returns an \Rclass{ExonCountSet} object with all the relevant information for 
