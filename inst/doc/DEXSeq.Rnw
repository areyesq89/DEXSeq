...
in comparative RNA-Seq experiments. By ``differential exon usage'', we mean that the fraction of
transcripts that incorporate the exon is affected by the experimental condition. The statistical 
method used by \Rpackage{DEXSeq} is described in our paper \cite{DEXSeqPaper}. The basic concept can 
be summarized as follows: 

For each exon (or part of an exon) and each sample, we count how many 
reads map to this exon and how many reads map to any of the other exons of the same gene. The ratio
of these two counts can be considered as a quantification of the exons ``usage'', i.e., it is a value
that is related to the fraction of the gene's transcripts that contain the exon. If the ratio
differs between, say, control and treatment samples, this indicates that the treatment affects
the exon's usage. In case of an inner exon, a change in the exon's usage is typically due to a change
in the rate with which this exon is spliced into transcripts (alternative splicing). The purpose of a 
DEXSeq analysis is to find such cases of ``differential exon usage'' (DEU). Note that DEU is more
general than alternative splicing: While changes in the usage of splice sites will typically affect
the inclusion of inner exons into transcripts, changes in the usage of alternative transcript start sites
and polyadenylation sites are likely of equal biological importance, and these will typically cause
differential usage of 5' and 3' boundary exons.

Similar to the case of differential gene expression, we need to make sure that the 
difference between treatment and control is statistically significant, i.e., that an observed
differential exon usage is sufficiently unlikely to be merely due to the random fluctuation that 
one also sees between identically treated samples. To this end, DEXSeq assesses the strength 
of these fluctuation (quantified by the so-called ``dispersion'') by comparing identically 
treated samples (replicates) before comparing the averages between the sample groups.

The explanation in the previous paragraphs is somewhat simplified to be accessible to users
withou statistics knowledge, and we urge users interested in the method to read the paper 
\cite{DEXSeqPaper} as well as Appendix \ref{changes} of the present documents, which explains the differences
between the present implementation of DEXSeq and the original method described in the paper. 
Two points should be made right here: Two, \emph{DEXSeq} does not work 
on the ratios but on the raw read counts, to be able to make use of the information that is
contained in the magnitude of count values (3000 reads versus 1000 reads is the same ratio as 3 reads
versus 1 read, but the latter ratio can be expected to be a far less precise estimate of the
true value, because of the low amount of data.) Second, \emph{DEXSeq} is not limited to
simple two-group comparisons. Rather, \emph{DEXSeq} is based on so-called generalized linear models
(GLMs) and hence any experimental design that is suitable for an ANOVA-like analysis can be
examined.
...
As an example to demonstrate the use of \emph{DEXSeq}, we use the \emph{pasilla} dataset, an
...
As in any RNA-Seq analysis, the first step is to align the reads to a reference genome. It is important
to align them to the genome, not to the transcriptome, and to use a splice-aware aligner (i.e., a short-read
...
we proceeded in preparing the pasilla data in the vignette for the \emph{pasilla} data package 
\cite{pasillaVignette}and a more extensive explanation, using the same example data set, in our 
protocol article on differential expression calling \cite{DEProt}.

\emph{Note:} Usually, Bioconductor vignettes contain executable code, i.e., you can follow the vignette 
by typing the code and it will run using data provided with the package. However, it would not have been
practical to include the rather voluminous raw data of the pasilla dataset with DEXSeq. Therefore, the
code in this section of the vignette is not directly executable. You may download the raw data yourself 
from GEO and then will be able to follow the work flow given here and in the pasilla vignette 
\cite{pasillaVignette}. From Section \ref{stdAnalysis} on, code is directly executable, as usual. 
Therefore, we recommend that your just read this section, and try following our analysis in R only from
the next section onwards. Once you work with your own data, you will want to come back and adapt the 
work-flow from here on.

\subsection{Two Python scripts}\label{PythonScripts}

The first two steps in a \emph{DEXSeq} analysis are done outside R, by using Python scripts. You do not
need to know how to use Python; however you have to install the Python package \emph{HTSeq}, following the 
explanations given on the HTSeq web page: 

\url{http://www-huber.embl.de/users/anders/HTSeq/doc/install.html}

An alternative work flow, which replaces the two Python-based steps with R scripts, has recently made available
and is shown in the \emph{parathyroidSE} vignette \cite{parathyroidSEVignette}. However, we proceed here
with the exposition of the default approach using Python. 

Once you have installed \emph{HTSeq}, you can use the two Python scripts, \verb|dexseq_prepare_annotation.py|
and \verb|dexseq_count.py|. These two files come as part of \emph{DEXSeq}. If you have trouble 
finding them, start R and ask for the installation directory with

<<systemFile>>=
system.file( "python_scripts", package="DEXSeq" )
@

The displayed path should contain the two files. If it does not, try to re-install \emph{DEXSeq} (as usual, with \Rfunction{biocLite}).

\subsection{Preparing the annotation} \label{prepAnno}

For the first step, you will need a GTF file with gene models for your species. If possible, use a GTF file
from Ensembl, as those of other providers (such as the UCSC Table Browser) often do not adhere fully to the GTF standard, which then causes preprocessing to fail. Ensembl GTF file can be found in the ``FTP Download'' sections of
...
sources to make the suitable as input for the Python script.

In a GTF file, most exon appear multiple times, namely once for each transcript that contains them. We
need to ``collapse'' this information to define ``exon counting bins'', i.e., a list of intervals, each corresponding to one exon, or part of an exon. Counting bins for parts of exons arise when an exonic
region appears with different boundaries in different transcripts. See Fig.~1 of the DEXSeq paper \cite{DEXSeqPaper}
for an illustration.

The Python script \verb|dexseq_prepare_annotation.py| takes an Ensembl GTF file and translates it into
a GFF file with ``flattened'' or ``collapsed'' exon counting bins.

Make sure that your current working directory contains the GTF file, then call the script 
...
In this command, which should be entered as a single line, replace \verb|/path/to|...\verb|/python_scripts| with the correct path to the Python scripts, which you have found with the \Rfunction{system.file} function (see above). 
\verb|Drosophila_melanogaster.BDGP5.72.gtf| is the Ensembl GTF file
(here the one for the fruit fly, already de-compressed) and \verb|Dmel_flattenend.gff| is the file name that the script will use to write its results to.
...
disable aggregation with the option ``\verb|-r no|''. Without aggregation, exons that overlap with other exon from
...
This command (again, to be entered as a single line) expects two file in the current working directory, namely the GFF file produced in the previous
...
If your data is from a \emph{paired-end} sequencing run, you need to take care of two points: First, the SAM file
needs to be sorted by read name. (Use \verb|samtools sort -n| for this purpose, and see Refs. \cite{pasillaVignette} and
\cite{DEProt} for further explanations, if needed.) Second, add the option ``\verb|-p yes|'' to 
the command to call the script. (As usual, options should be placed before the file names.)
...
The remainder of the analysis process it now done within R. Start R and load \Rpackage{DEXSeq}
...
more prudent to prepare the table in a spread-sheet program such as Excel (or even better,
...
the experimental condition (factor \texttt{condition} with levels \texttt{control} and
\texttt{treatment}) and the library type (factor \texttt{libType}), which we included because
the samples in this particular experiment have been sequenced partly in single-end
runs and partly in paired-end runs. 

We will, however, ignore this latter information in the 
analysis described in the next section and postpone the discussion on how to include
such additional covariates to section \ref{sec:glm}. If you have only a single covariate and
want to perform a simple default analysis, you the column with this factor should be
named \emph{condition}.
...
The function takes three argument. First, a vector with names of count files, i.e., of files
...
There are other ways to get a \emph{DEXSeq} analysis started. See Appendix \ref{sec:creatingInR} and 
...
To demonstrate the \emph{DEXSeq} work flow with actual data, we have prepared, in the \Rpackage{pasilla}
data package, an \Robject{ExonCountSet}, similar to the one constructed in the previous section.
However, in order to keep the run-time of the vignette small, we have subset the object to
only a few genes.

To start, we load the packages \Rpackage{DEXSeq} load the
example data from the \Rpackage{pasilla} package with the \Rfunction{data} command.
...
The object \Robject{pasillaExons} is an ExonCountSet, which has been constructed with 
...
The core data in such an \Rclass{ExonCountSet} object are the counts 
per exon. Let's have a look at the first 20 rows.
<<seeCounts>>=
head( counts(ecs), 20 )
@
...
So far, these columns contains information on the annotation data, such as gene and exon IDs,
genomic coordinates of the exon, and the list of transcripts that contain an exon. Further
columns for intermediate and final results are already present but will be filled in only
in the subsequent steps of the analysis.

In the following sections, we will update the object by calling a number of analysis function,
...
condition, as indicated in the \emph{conditions} factor of the design table (see above), are 
...
design which cannot be accommodated by a single ``condition'' factor.
...
likelihood estimation (our method follows that of the package \Rpackage{edgeR}~\cite{edgeR_GLM}).
Before starting estimating the CR dispersion estimates, \Rfunction{estimateDispersions}
first defines the ``testable'' exons, which fulfil the following criteria:
\begin{enumerate}
...
The following command performs these steps.
...
\incfig{DEXSeq-fitdiagnostics}{.5\textwidth}{Fit Diagnostics.}{
...
\end{equation}
In these formulas (which use the standard notation for linear model formulas in R; see any text book
on R if you are unfamiliar with it), \verb|sample| is factor with different levels for each sample,
\verb|condition| is the factor of experimental conditions that we defined when constructing the
\Rclass{ExonCountSet} object at the beginning of the analysis, and \verb|exon| is a factor with
two levels, \verb|this| and \verb|others|. The two models described by these formulas are fitted for
each exon, where the data supplied to the fit comprised two read count values for each sample, namely
the number of reads mapping to the exon in question, and the sum of the read counts from all other exons
of the same gene. Note that this approach differs from the approach described in our paper \cite{DEXSeqPaper}
and used in older versions of \Rpackage{DEXSeq}; see Appendix \ref{changes} for further discussion.

Readers familiar with linear model formulas might find one aspect of Equation (\ref{eq:altmodel}) surprising
at first sight: We seem to have an interaction term \verb|condition:exon|, but no main effect for \verb|condition|.
...
factor can be seen as nested within the \verb|condition| factor.
...
contain interesting cases; so the large fraction of affected genes seen here is not typical.
...
then have to indicate the design by providing explicit formulas for the test.

In the \emph{pasilla} dataset, some samples were sequenced in single-end and others in paired-end mode. Possibly,
...
We specify two design formulas, which indicate that the \verb|libType| factor should be treated
...
Compare these formulas with the default formulas given in Section \ref{sec:deu}. We have added, in both 
the full model and the reduced model, the term |\verb|type : exon|. Therefore, any dependence of exon
usage on library type will be absorbed by this term and hence accounted for equally in the full
...
To start a fresh analysis, now using these formulas instead of the default ones, we copy the example 
...
The test function now needs to be informed about both formulas
...
@
Accounting for the library type has improved power: we found one more hit. (Of course, one
more hit is hardly convincing: however, if one performs the comparison on the full
pasilla data, the improvement is more noticeable.)
...
As explained in detail in the paper, \Rpackage{DEXSeq} is designed to find difference in exon usage, i.e. differences in expression
...
will not be considered evidence of differential exon usage. Hence, it may be advantageous
...
\Rfunction{DEUresultTable}. In the following, we also specify design formulas
...
have long run time. Therefore, no command output is given.]
...
Next, we ``flattened'' gene model to get counting bins (as done by the \verb|dexseq_prepare_anotation.py|
script in the standard approach, Section \ref{prepAnno}).
...
plays the same role as the \verb|-r| option to \verb|dexseq_prepare_anotation.py| described at the 
...
in order to build our typical \Robject{ExonCountSet} object. The middle argument in
the following call is the design, indicating that the first two BAM files form
one experimental condition and the third one the other. More typically, you may want
to pass a data frame with a sample table as in Section \ref{sec:readData}.
...
We list them here breifly; see the help pages of these function for further options (e.\,g., to specify formulae).
...
in the formulas passed to \Rfunction{estimateDispersions} and \Rfunction{testForDEU} refer 
...
the counts for all the gene's exons. However, this turned out to increase computation time
considerably for genes with many exons because the many exons require very large model matrices,
which are computationally expensive to deal with. We have therefore modified our approach: when fitting
a model for an exon, we now sum up the counts from all the other exon and use only the total, rather than
the individual counts in the model. Now, computation time is independent of the number of exons per gene,
which improved \emph{DEXSeq}'s scalability. While the $p$ values returned by the two approaches are not
exactly equal, the differences were only very minor in the tests that we performed.

For now, the function for our original approach (which we now call the ``big model'' or ``BM'' approach)
are still included; all relevant functions, however, have been renamed to now carry the suffix \verb|_BM|
in their name. The new approach, which is now default and is used by the work flow described in this 
vignette, has no special name; however, in previous releases of \emph{DEXSeq}, which had included it 
...
of the read counts $K_{i'j0}$ from all other counting bins $i'$ in the same gene. Hence, when we write $K_{ijl}$, 
$l$ indicates whether we mean the read count for bin $i$ ($l=0$) or the sum of counts for all other bins of the 
same gene ($l=1$).

As before, we fit a GLM of the negative binomial (NB) family
\begin{equation} K_{ijl} \sim \operatorname{NB}(\text{mean}=s_j\mu_{ijl},\text{dispersion}=\alpha_i), \end{equation}
now with the model specified in Equation (\ref{eq:altmodel}), which we write out as
\begin{equation} \log_2 \mu_{ijl} = \beta^\text{S}_{ij} + l \beta^\text{E}_{i} + \beta^\text{EC}_{i\rho_j}. \end{equation}
...
using the full model given above. For the likelihood ratio test, this full model is fitted and compared with the fit
or the reduced model, which lacks the interaction term $\beta^\text{EC}_{i\rho_j}$. As described in \ref{sec:glm},
alternative model formulas can be specified.
...
is used to convert a GTF file with gene models into a GFF file with ``flattened'' gene models. We recommend to
...
convert it to the expected format, typically using a custom-made script. To this end, we here give details on
how the \verb|dexseq_prepare_annotation.py| script inprets a GFF file.

\begin{itemize}
\item The script only looks at ``exon'' lines, i.e., at lines which contain the word ``\verb|exon|'' in the 
...
(and not ``\verb|Parent|'' as in GFF3 files, or \verb|GeneID| as in some older GFF files), and it must give the same ID to 
all exons from the same genes, even if they are from different transcripts of this gene. (This last condition is violated 
by GTF files generated by the Table Browser function of the UCSC Genome Browser.)
\item The \verb|transcript_id| attribute is used to build the \verb|transcripts| attribute in the flattened GFF file, 
...
that the exon lines have type ``\verb|exon|'' and that the atributes giving gene ID (or gene symbol) and transcript ID are 
called ``\verb|gene_id|'' and ``\verb|transcript_id|'', in this spelling. Remember to also take care that the chromosome 
