...
(here the one for the fruit fly, already de-compressed) and \verb|Dmel_flattenend.gff| is the name of the output file.
...
To demonstrate the \Rpackage{DEXSeq} work flow with actual data, we have prepared, in the \Rpackage{pasilla}
...
vignette, has no special name; however, in previous releases of \Rpackage{DEXSeq}, which had included it 
