...
\usepackage{Sweave}
\SweaveOpts{keep.source=TRUE, eps=FALSE, pdf=TRUE, png=FALSE, include=FALSE, width=4, height=4.5, prefix=FALSE}
...
diffential exon usage is sufficiently unlikely to be merely due to the random fluctuation that 
...
simple two-group comparisons. Rather, \emph{DEXSeq} is based on so-called generalized linear mdoels
(GLMs) and hence any experimental design that is suitable for an ANOVA-like analysis can be
examined.

\subsection{Workflow}

The input for a \emph{DEXSeq} analysis is a count table, with one column for each sample, and
one row for each \emph{couting bin}, where the latter term refers to an exon, or part of an exon.
The table fields contains read counts, i.e., integer numbers which indicate how many sequencing
reads were aligned to the given counting bin in the given sample. There are different ways to obtain
such a count table: the recommended approach for standard situations is described in Section 
\ref{PythonScripts}, an alternative approach is documented in Appendix \ref{sec:creatingInR}.

From this input data, an R object of class \Rclass{ExonCountSet} is constructed. In the 
subsequent analysis steps, this object is filled with intermediate and final results.
...
of teh analysis work-flow presented here starts with the aligned reads, in the SAM format. If you
...
\emph{Note:} Usually, Bioconductor vignettes containe executable code, i.e., you can follow the vignette 
by typing the code and it will run using data provided with the package. However, it would not have been
practical to include the rather voluminuous raw data of the pasilla dataset with DEXSeq. Therefore, the
...
Therfore, we recommend that your just read this section, and try following our analysis in R only from
...
An alternative workflow, which replaces the two Python-based steps with R scripts, has recently made available
...
In a GTF file, most exon appear mutiple times, namely once for each transcript that contains them. We
...
In this command, which should be enetered as a single line, replace \verb|/path/to|...\verb|/python_scripts| with the correct path to the Python scripts, which you have found with the \Rfunction{system.file} function (see above). 
...
By default, the countings script assumes your library to be \emph{strand-specific}, i.e., reads are aligned to
the same strand as the gene they originate from. If yor have used a library preparation protcol that does
...
all relevant information such as name of the file with the read counts, experimental conditons, technical
...
take an existing table that had already been prepared when the eperiments were done),
...
\texttt{treatment}) and the librart type (factor \texttt{libType}), which we included because
...
The function takes three agrument. First, a vector with names of count files, i.e., of files
that have been generated with the \verb|dexseq_count.py| script. The function will read these
files and arrange the count data in a matrxi, which is stored in the \Robject{ExonCountSet}
...
metadata slot (see below). The third argument is a file name again, now of the flattened GFF file
...
To demonstrate the \emph{DEXSeq} workflow with actual data, we have prepared, in the \Rpackage{pasilla}
...
To start, we load the packages \Rpackage{DEXSeq} and \Rpackage{pasilla} and load the
example data with the \Rfunction{data} command.
...
in the subsequents steps of the analysis.
...
\fixme{Make sure these are in the package, then remove this section.}
...
In this section, we discuss simple one-way designs: In this setting, samples with the same experimental condition, as indicated in the \emph{conditions} factor of the design table (see above), are 
...
design which cannot be accomodated by a single ``condition'' factor.
...
\incfig{fitdiagnostics}{.5\textwidth}{Fit Diagnostics.}{
...
\mbox{\texttt{\tttildemiddle\ sample + exon * condition}}
\end{equation}
and compare it to the smaller model (the null model)
\begin{equation}\label{eq:nullmodel}
\mbox{\texttt{\tttildemiddle\ sample + condition}.}
\end{equation}
In these formulas (which use the standard notation for linear mdoel formulas in R; see any text book
on R if you are unfamiliar with it), \verb|sample| is factor with different levels for each sample,
\verb|condition| is the factor of experimental conditions that we defined when constrcuting the
...
model~(\ref{eq:altmodel}), which contains an interaction coefficient for \verb|exon:condition|. The latter
...
A convenient way to extract all relevant columsn of the feature data table is
...
Mean expression versus $\log_2$ fold change plot. Significant hits (at \Robject{padj}<0.1) are colored in red.
}
\fixme{Make MA plots nicer.}
...
to one of two experimental conditions. If your experiment is of this type, you can use the workflow shown above. All
...
this influenced counts and should hence be accounted for. We therefore use this as an example for a compelx design.
...
<<formulas2>>=
formulaFullModel    <-  ~ sample + type * exon + condition * exon
formulaReducedModel <-  ~ sample + type * exon 
@
...
usage on library type will be absorbed by these terms and hence accounted for equally in the full
...
pasilla data, the improvement is mroe noticeable.)
...
pasillaExons <- doCompleteDEUAnalysis(
   pasillaExons,
   formula0 =  ~ sample + exon * type,
   formula1 =  ~ sample + exon * type + exon * condition,
   nCores = 1 )
@
<<alldeuout>>=
...
\Rpackage{parathyroidSE} package \cite{parathyroidSEVignette}. The workflow is similar to the one 
...
Next, we ``flattened'' gene model to getcounting bins (as done by the \verb|dexseq_prepare_anotation.py|
...
plays the same role as the \verb|-r| option to \verb|dexseq_prepare_anotation.py| descibed at the 
...
\Rfunction{countReadsForDEXSeq}. To demonstarte this, we first determine the patrhs to the
...
used for the dispersion fits and the mdoel fits involved in the likelihood ratio test. The terms used
