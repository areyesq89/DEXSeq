...
of the feature data. Then we call the function \Rfunction{fitDispersions}, in which a dispersion-mean 
relation $\alpha = \alpha_0 + \alpha_1/\mu$ is fit to the CR dispersion values (and the coefficients 
stored in slot \Robject{dispFitCoefs}) and finally, for each exon, the maximum of the CR estimate and the fitted value
is taken as the exon's final dispersion value and stored in the \Robject{dispersion} slot.
Please take into consideration that fit could be difficult to do in cases where the dispersion estimates
are too big, this sometimes tends to break this function.  In this special cases please do not hesitate to 
...
To make a fit diagnostic, for each exon we plot its mean expression value vs its CR
dispersion estimate and we draw the line that we just fitted.

<<fitdiagnostics, fig=TRUE>>=
meanvalues <- rowMeans(counts(pasillaExons))
plot(meanvalues, fData(pasillaExons)$dispersion_CR_est, log="xy", main="mean vs CR dispersion")
...
We can see that the CR dispersion estimates depends on its mean value. The line seems to pass through 
the data points, which gives us a good diagnostic of the fit.\fixme{Comment something about the lower cloud of points}
...
\Rfunction{testForDEU}) by changing the parameter n.cores in the functions.  Internally, this functions 
will subset the \Robject{ExonCountSet} object into smaller objects to distribute them to different cores.  
...
the user can do the subsetting manually in order to paralellized with other packages, e.g. \Rpackage{Rmpi}.
%
<<para1,cache=TRUE,eval=FALSE>>=
data("pasillaExons", package="pasilla")
library(multicore)
pasillaExons <- estimateSizeFactors( pasillaExons )
pasillaExons <- estimateDispersions( pasillaExons, n.cores=3, quiet=TRUE)
pasillaExons <- testForDEU( pasillaExons, n.cores=3)
@
