...
\begin{abstract}
  ...
  \Rpackage{pasilla}. For citation purposes, please refer to
...
Then, to test for differential expression, we need to estimate the variance of the data. This is needed
in order to be able to distinguish between normal technical and biological variation (noise) from real effects
...
The function we just fitted is plotted in the panel and it is passing through the data points. This gives
...
an additional experimental variable as well.
<<design>>=
design(pasillaExons)
@
...
%--------------------------------------------------
DEXSeq analysis can be computationally heavy with large datasets due to the number of iterations and glm's
...
dependent fit is done with all the dataset.  The nCores parameter depends on the \Rpackage{multicore} package, but
...
%--------------------------------------------------
In the previous sections, we passed through each of the steps to make an analysis for differential exon usage using \Rpackage{DEXSeq}.
